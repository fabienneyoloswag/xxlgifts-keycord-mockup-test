...
async function exportPng(){
 ...
 try{
  ...
  if(!blob)throw new Error('PNG kon niet worden opgebouwd.');

  const url=URL.createObjectURL(blob);
  const link=document.createElement('a');
  link.href=url;
  link.download=`xxlgifts-${currentTemplate?.id||'keycord'}-mockup.png`;
  ...
  console.error(error);
  alert('De PNG kon niet worden gedownload. Vernieuw de pagina en probeer opnieuw.');
 }finally{
  if(previousEditing){
   editing=true;
   render();
